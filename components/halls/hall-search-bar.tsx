              ...
              <div className="flex flex-wrap gap-2">
                {CAPACITY_OPTIONS.map((opt) => (
                  <button
                    ...
                    style={filters.minCapacity === opt.value ? CHIP_ACTIVE : CHIP_IDLE}
                  >
                    {opt.label}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {PRICE_OPTIONS.map((opt) => (
                  <button
                    ...
                    className="rounded-lg px-3 py-1.5 text-xs font-semibold font-body transition-all"
                    style={filters.maxPrice === opt.value ? CHIP_ACTIVE : CHIP_IDLE}
                  >
                    {opt.label}
                  </button>
                ))}
            ...
            <div className="flex flex-wrap gap-2">
              {allFeatures.map((feature) => (
                <button
                  ...
                  className="rounded-lg px-3 py-1.5 text-xs font-semibold font-body transition-all"
                  style={filters.features.includes(feature) ? CHIP_ACTIVE : CHIP_IDLE}
                >
                  {feature}
                </button>
              ))}
