...
export function CalendarPanel({ onClose }: CalendarPanelProps) {
  ...
  const MONTHS = Array.from({ length: 12 }, (_, i) => t(`months.${i}` as Parameters<typeof t>[0]))
  const DAYS = Array.from({ length: 7 }, (_, i) => t(`days.${i}` as Parameters<typeof t>[0]))
      ...
      >
        ...
        >
          ...
          {Object.keys(bookingDates).length > 0 && !selectedDay && (
            ...
            >
              <p className="font-display text-sm font-semibold text-foreground mb-3">
                {t('calendar.thisMonth')} — {Object.values(bookingDates).flat().length}{' '}
                {t('calendar.bookingsSuffix')}
              </p>
