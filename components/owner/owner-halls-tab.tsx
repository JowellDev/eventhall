		...
		<div>
			...
			{halls.length === 0 ? (
				<div className="flex flex-col items-center justify-center py-20 text-center">
					<p className="text-muted-foreground font-body text-sm">{t.noHalls}</p>
				</div>
			) : (
				<>
								...
								<div className="p-4">
									...
									<div className="flex items-center gap-2">
										...
										<button
											...
											aria-label={t.deleteHallLabel(hall.name)}
										>
			...
			{selectedHall && <OwnerHallDetailModal hall={selectedHall} onClose={() => setSelectedHall(null)} />}
			{editHall && <AddHallModal hall={editHall} onClose={() => setEditHall(null)} />}
			{deleteTarget && (
				<ConfirmDialog
					title={t.deleteHallLabel(deleteTarget.name)}
					message={
						<>
					...
					onConfirm={() => { deleteHall(deleteTarget.id); setDeleteTarget(null) }}
				/>
			)}
		</div>
	)
