...
export function OwnerBookingsTab() {
	...
	return (
		<div>
							...
							<div className="flex-1">
								...
								{booking.packages.length > 0 && (
									<p className="text-xs text-muted-foreground font-body mt-1">
										{t.optionsLabel} {booking.packages.join(', ')}
									</p>
								)}
							</div>
							<div className="flex items-center gap-3">
								...
								{booking.status === 'pending' && (
									<div className="flex gap-2">
										...
										>
											<XCircle className="w-3.5 h-3.5" /> {t.refuse}
										</button>
									</div>
								)}
							</div>
			...
			{pendingAction && (
				<ConfirmDialog
					title={pendingAction.action === 'confirmed' ? t.confirmBookingTitle : t.refuseBookingTitle}
					message={
						<>
							{pendingAction.action === 'confirmed'
								? t.confirmBookingMsg(pendingAction.booking.client, pendingAction.booking.hallName, pendingAction.booking.date)
								: t.refuseBookingMsg(pendingAction.booking.client, pendingAction.booking.hallName, pendingAction.booking.date)}
						</>
					}
					confirmLabel={pendingAction.action === 'confirmed' ? t.confirm : t.refuse}
