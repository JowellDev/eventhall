...
type View = 'login' | 'register' | 'forgot-1' | 'forgot-2' | 'forgot-3' | 'forgot-success'
...
function GoldButton({ loading, label, loadingLabel }: { loading: boolean; label: string; loadingLabel: string }) {
  return (
    <button
      ...
      style={{ background: 'linear-gradient(135deg, #d4af37, #f4c430)', color: '#000', opacity: loading ? 0.7 : 1 }}
    >
      {loading ? (
...
export function LoginModal({ open, onClose, onLogin }: LoginModalProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    await new Promise((r) => setTimeout(r, 600))
    const user = addUser({ name: regName, email: regEmail, password: regPassword, role: regRole })
  ...
  const logoSection = (
    <div className="flex flex-col items-center mb-6">
      ...
      <h2 className="font-display font-bold text-2xl gold-gradient-text">EventHalls</h2>
    </div>
  )
        ...
        {view === 'login' && (
          <>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <GoldButton loading={loading} label="Se connecter" loadingLabel="Connexion..." />
            </form>

            <div className="mt-5 pt-5 border-t text-center" style={{ borderColor: 'rgba(212,175,55,0.1)' }}>
            ...
            <div className="mt-5 pt-5 border-t" style={{ borderColor: 'rgba(212,175,55,0.1)' }}>
              <p className="text-xs text-muted-foreground font-body mb-3 text-center">
                Comptes de démonstration
              </p>
              <div className="space-y-1.5">
                {[
                  { email: 'client@eventhalls.com', password: 'Client@123', label: 'Client' },
                  { email: 'owner@eventhalls.com', password: 'Owner@123', label: 'Propriétaire' },
                  { email: 'admin@eventhalls.com', password: 'Admin@123', label: 'Administrateur' },
                ].map((u) => (
                  <button
                    key={u.label}
                    type="button"
          ...
          <>
            ...
            <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {(['client', 'owner'] as const).map((r) => (
                    <button
                      ...
                      style={{
                        borderColor: regRole === r ? '#d4af37' : 'rgba(212,175,55,0.2)',
                        background: regRole === r ? 'rgba(212,175,55,0.12)' : 'transparent',
                        color: regRole === r ? '#d4af37' : 'var(--muted-foreground)',
                      }}
                    >
              ...
              <div>
                ...
                <PasswordInput value={regPassword} onChange={setRegPassword} placeholder="Min. 8 caractères" />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5 font-body">
                  Confirmer le mot de passe
                </label>
                <PasswordInput value={regConfirm} onChange={setRegConfirm} placeholder="••••••••" />
              </div>
              {error && <ErrorMsg msg={error} />}
              <GoldButton loading={loading} label="Créer mon compte" loadingLabel="Création..." />
            </form>
          </>
        )}
        ...
        {view === 'forgot-1' && (
          <>
            ...
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 className="font-display font-bold text-xl text-foreground">Mot de passe oublié</h2>
            ...
            <div className="flex justify-center gap-2 mb-6">
              {[1, 2, 3].map((s) => (
                <div
                  ...
                  style={{ background: s === 1 ? '#d4af37' : 'rgba(212,175,55,0.2)' }}
                />
              ))}
            </div>
            <form onSubmit={handleForgot1} className="space-y-4">
              ...
              {error && <ErrorMsg msg={error} />}
              <GoldButton loading={loading} label="Envoyer le code" loadingLabel="Envoi en cours..." />
            </form>
          </>
        )}
        ...
        {view === 'forgot-2' && (
          <>
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 className="font-display font-bold text-xl text-foreground">Vérification</h2>
              <p className="text-muted-foreground text-sm mt-1 text-center font-body">
            ...
            <div className="flex justify-center gap-2 mb-6">
              {[1, 2, 3].map((s) => (
                <div
                  key={s}
                  className="w-8 h-1.5 rounded-full"
                  style={{ background: s <= 2 ? '#d4af37' : 'rgba(212,175,55,0.2)' }}
                />
              ))}
            </div>
            <div
            ...
            <form onSubmit={handleForgot2} className="space-y-4">
              ...
              {error && <ErrorMsg msg={error} />}
              <GoldButton loading={false} label="Vérifier le code" loadingLabel="Vérification..." />
            </form>
            <button
              onClick={() => go('forgot-1')}
        ...
        {view === 'forgot-3' && (
          <>
            <div className="flex flex-col items-center mb-6">
              ...
              <h2 className="font-display font-bold text-xl text-foreground">Nouveau mot de passe</h2>
              <p className="text-muted-foreground text-sm mt-1 text-center font-body">
            ...
            <form onSubmit={handleForgot3} className="space-y-4">
              <div>
                ...
                <PasswordInput value={fpPassword} onChange={setFpPassword} placeholder="Min. 8 caractères" />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground mb-1.5 font-body">
                  Confirmer le mot de passe
                </label>
                <PasswordInput value={fpConfirm} onChange={setFpConfirm} placeholder="••••••••" />
              </div>
              {error && <ErrorMsg msg={error} />}
              <GoldButton loading={loading} label="Réinitialiser" loadingLabel="Enregistrement..." />
            </form>
          </>
        )}
